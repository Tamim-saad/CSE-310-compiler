...
public:
  ...
  void addParameter(const string &name) { parametreList.push_back(name); }

  int getNumberOfParameters() { return parametreList.size(); }
...
public:
  ...
  IdInfo(const string &name, const string &type, int sz)
      : SymbolInfo(name, "ID") {
    idType = type;
    size = sz;
    dataType = type;
